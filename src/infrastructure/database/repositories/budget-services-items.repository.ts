...
@Injectable()
export class BudgetServicesItemsRepository extends AbstractRepository<BudgetServicesItems> {
  constructor(
    @Inject('DATA_SOURCE') dataSource: DataSource,

    private readonly logger: Logger,
  ) {
    super(dataSource.getRepository(BudgetServicesItems));
  }
  ...
  async findById(id: number) {
    ...
    return await this.repository.findOne({
      where: { id },
      relations: ['serviceItem', 'status', 'assignedUser', 'budgetItem', 'budgetItem.budget'],
    });
  }
  ...
  async save(budgetServicesItems: BudgetServicesItems) {
